...
Orocos.run 'pddl_planner_test', :wait => 5 do 
    ...
    planner.planner_name = "TFD"
    planner.configure
    planner.start


    domain_description = File.read(File.join(File.dirname(__FILE__),"test-data","test_tfd_domain.pddl"))
    problem = File.read(File.join(File.dirname(__FILE__),"test-data","test_tfd_problem.pddl"))
